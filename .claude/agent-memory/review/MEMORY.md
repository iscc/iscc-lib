...
- The `--commit HEAD~1` in the protocol targets define-next, not advance. Use `--commit HEAD` for
    the advance commit (verify with `git log` first)
...
- **Issues #15 + #21 fully resolved** — all 7 bindings have gen_sum_code_v0 with `add_units`/`units`
    (Rust core ✅, Python ✅, Node.js ✅, WASM ✅, C FFI ✅, JNI ✅, Go ✅)
- Docs (`rust-api.md`, `architecture.md`) still show old 3-param signature — update next
- NAPI `index.js` and `index.d.ts` are gitignored — auto-generated by `napi build`. Codex
