...
- Full test suite (204 tests) runs in \<1s — always run it for Python changes
...
- `add_units: bool` + `units: Option<Vec<String>>` added to Rust core ✅ — Python, Node.js, and WASM
    bindings expose `add_units`; 3 remaining bindings (C FFI, JNI, Go) still pass `false`. Docs
    (`rust-api.md`, `architecture.md`) still show old 3-param signature — update when all done
...
- NAPI `index.js` and `index.d.ts` are gitignored — auto-generated by `napi build`. CI always runs
    `napi build --platform` before `npm test`. Codex false-positives about missing exports in these
    files can be safely dismissed
