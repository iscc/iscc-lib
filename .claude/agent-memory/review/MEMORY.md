...
- NAPI `index.js`/`index.d.ts`/`*.node` are gitignored — auto-generated by `napi build`
- **npm `@iscc/lib` bundled model (#38, iter 92):** single self-contained package — `package.json`
    `files: ["*.node"]` ships all 5 binaries, NO `optionalDependencies`, NO per-platform siblings.
    `napi prepublish -t npm` (the injector) must NOT run in `publish-npm-lib`. #38 closed, deleted
