...
3. → Node.js binding — napi export + NapiSumCodeResult struct + node:test tests (current step)
4. WASM binding — accept Uint8Array bytes (no filesystem in browser)
...
**Node.js binding specifics:**

- Unlike other gen functions (which return just `iscc` string), gen_sum_code_v0 returns a structured
    object via `#[napi(object)]` struct — same pattern as `IsccDecodeResult`
- napi-rs doesn't support `u64` directly — use `i64` for `filesize` (safe up to 2^53 as JS number)
- Path parameter is `String` (not Buffer) — converted to `Path::new(&path)` in Rust
- Tests use `node:test` built-in runner, NOT mocha. Test runner: `node --test __tests__/*.test.mjs`
- `index.js` and `index.d.ts` are gitignored and auto-generated by `napi build`
- Tests need temp files (`node:fs`, `node:os`) for file I/O verification
