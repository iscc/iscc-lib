...
## Current State (assessed-at: 8930cba, iter 122)

- **IN_PROGRESS — CI RED on develop tip.** v0.5.0 released (`0.5.0`), all 12 bindings meet CORE
    criteria. Rust-core `iscc_decode` trailing-byte fix LANDED (iter 121, both branches lib.rs
    L235+L241, new test ~L2029, review PASS) — BUT it broke CI.
- **CI RED on origin/develop tip `880ec5e`** (HEAD `8930cba` = +1 log-only commit). ONLY failing job
    = **`Coverage + CRAP`** (both workflow runs 30113161728 + 30113159676). Root cause = CRAP
    `--fail-regression` `↑ 1 regressed`: `iscc_decode`'s new branch raised cyclomatic above baseline
    (was 4.0/crap 4.11 in `.crap-baseline.json` ~L277). All other 20 check-runs `success`. FIX =
    refresh that baseline entry (legit complexity, far \<30 `--fail-above`); NO source revert.
    Slipped through because CRAP regression gate is CI-only, not in `mise run check` (see Quality
    Gates).
- **cargo-deny gate enforcing** — see Quality Gates. Live advisory can re-red any push (prefer
    `cargo update -p` over deny.toml ignore).
- **v0.6.0 remaining (`normal` `[human]`, spec'd)**: #49 aarch64 wheels, dependency review/refresh →
    Python/CI-CD **partially met**.
- **6 issues: 0 critical, 4 normal `[human]`, 2 low `[human]`.** normal: aarch64 wheels, dep
    ...
    issue open (the Rust-core trailing-byte one was resolved+deleted iter 121). The CRAP CI failure
    is NOT an issues.md entry — fix it directly as top priority.
- **MET sections**: Node, WASM, C FFI, Java, Go, Ruby, .NET, C++, UniFFI, Swift, Kotlin, README,
    per-crate READMEs, Docs, Benchmarks. Rust-core + Python + CI/CD = partially met.
- **Don't re-flag as new work**: Rust-core iscc_decode trailing-byte (iter 121), Go IsccDecode
    trailing-byte (120), #43 Go ISCC-IDv1 (119), #42 WASM SIMD (118), GIL #41 (116), cargo-deny
    gate, CRAP `--fail-above` (113), iai perf gate (107-111), PyO3 #1 (105), semver gate (93), npm
    #38, GIL #39, SumHasher #37. CID infra (audit role, `metrics.jsonl`, `decisions.md`, scope
