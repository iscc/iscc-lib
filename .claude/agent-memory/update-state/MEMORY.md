...
- **Counts** (re-verified 155): crate/pkg READMEs & CLAUDE.md 12 each (**scope the glob to
...
- **Unicode = 16.0.0 + SENTINEL freeze rule → read `unicode-contract.md` before ANY Unicode call.**
    Crit 1+2 MET (148); crit 3 at **8 of 11 native surfaces** (Rust, Python, WASM, Ruby, napi, Java,
    C#, Kotlin) plus the pure-Go port, leaving **C FFI, C++, Swift + 4 sibling `data.json`**; crit 4
    (differential sweep) has no harness. `decisions.md` keeps 2 SUPERSEDED designs — do NOT
    implement.
- **Per-surface fixture-loading plumbing table → `unicode-contract.md`** (read it before costing any
    propagation slice). Inline landmark: **surefire's default CWD is the pom basedir**, NOT the
    `mvn -f` dir — that is why the Java suites' `../../iscc-lib/tests/…` paths resolve.
...
- **Reproduce/refute binding claims yourself** — cheap probes beat inherited text (confirmed a
    `critical` bug at 147, refuted a stale-napi-`.node` claim at 151-153). Deliberate deviations
...
- **Cost-rank a propagation slice on THREE axes** (153, extended 154): fixture-reading plumbing
    ...
    iterations; the third axis is what demotes Swift/C++ (no toolchain here → CI-only proof).
- **A fourth question, surfaced at 154: is the fixture a DECLARED INPUT of that build system?**
    Gradle/MSBuild/CMake incremental checks can report UP-TO-DATE and skip the whole suite. Every
    surface-level gate was green while 13 Kotlin tests silently did not run; only a
    mutate-then-rerun probe (no `clean`) exposes it. CI is immune (fresh checkout) — this bites
    local/CID verification.
...
## Current State (assessed-at: 0b6b3be, iter 155)

- **IN_PROGRESS — CI GREEN AND COVERING ALL CODE.** `origin/develop` == `ceb32fd`: 43 check-runs, 22
    names, 0 non-success. HEAD `0b6b3be` is ONE `cid(log)` commit ahead but the non-`.claude` diff
    vs origin is EMPTY. ~2x runs because PR **#44 (develop→main) is OPEN** ("Release 0.6.0" — NOT
    shipped; version **0.5.0**). Iteration 154 ran clean (4 roles `OK`, verdict `PASS_WITH_NOTES`).
- **Statuses:** Rust-core partially met (crit 3 at **8 of 11** surfaces + the pure-Go port; crit 4
    not started); all else met except CI/CD (partial). All 8 gated suites ride EXISTING CI jobs
    (`python-test`, `nodejs`, `wasm`, `java`, `ruby`, `dotnet`, `kotlin`, `go`) — each rebuilds its
    native first. Per-section evidence lives in state.md — don't duplicate it here.
- **Invariant to re-check every propagation slice:**
    ...
    `git ls-files` set check + `>= 5` floor + ASCII guard, rides `python-test`). Slice 4 added NO
    copy (linked instead — ruled). Register every new copy; keep canonical basenames.
- **Next = criterion 4, the differential sweep** (only Unicode item with no partial credit, fully
    local, protects all surfaces). Must cover BOTH 1,112,064 scalars AND sequence classes; scope its
    blind spots (fail-open? zero-case = green? skipped vs passed? declared input?). Alternative =
    slice 5 **C FFI** (only remaining surface buildable here, but needs a generated C table or a
    hand-rolled reader — bare `gcc` compile, no JSON parser). Swift next-to-last (needs a TRACKED
    copy via SwiftPM `resources:`), C++ last (`cmake` absent).
...
- **Issues: 8** (4 `normal`, 4 `low`, 0 critical, 0 `HUMAN REVIEW REQUESTED`), unchanged at 154.
    **Human backlog CLEARED.** AUTHORIZED for CID: the rubygems `@v2.1.0` pin; major dep bumps **one
    ...
    Kotlin already warns 8.12.1 vs plugin 2.4.10 — JUnit 6.x); exhaustive `specs/ci-cd.md` job
    table. DEFERRED: npm OIDC.
- **Don't re-flag as DONE**: C#+Kotlin 154; napi+Java 153; drift gate 152; WASM+Ruby 151; Python+Go
    150; sequence vectors 149; sentinel conversion 148 (earlier → `MEMORY-archive.md`).
...
- **Container toolchains (re-probed 155):** `dotnet`, `mvn`, `gcc`, `java`, `go`, `ruby`, `node`,
    `wasm-pack` present; **`cmake`, `swift`, `gradle` ABSENT** → `packages/{cpp,swift}` are
    CI-verified only. **Kotlin IS locally testable despite no `gradle`** — iteration 154 ran
    `./gradlew -p packages/kotlin cleanTest test --offline` green (wrapper + warm cache). Probe with
    `command -v`; never assume from a package dir existing. Other env gotchas → `MEMORY-archive.md`.
