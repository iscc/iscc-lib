...
- **Counts** (re-verified 135): READMEs/CLAUDE.md 12 each; pytest-benchmark 18; UniFFI exports 32;
...
- **Unicode = DECLARED 16.0.0 + freeze rule** (human decision 132) → **`unicode-contract.md`**:
    ...
    `grep -rl "1FAE9\|113C5\|20C1" crates/ packages/ tests/ scripts/` → at 135 still only `utils.rs`
...
- **`[tool.ruff.lint]` since 134**: `extend-select = ["S", "C901"]`, still **NO `select` key** —
    that is load-bearing (a bare `select` would drop ruff's `E4`/`E7`/`E9`/`F` defaults). `S`/`C901`
    now fire at commit + `mise run lint` + CI; the 2 pre-push `--select` hooks are deliberate
    redundancy (`decisions.md` 2026-07-25). Live 0.16 count:
...
## Current State (assessed-at: 4cd963b, iter 135)

- **IN_PROGRESS — CI GREEN.** v0.5.0 released. Iter 134 was ruff slice B only (4 code-diff files,
    all tooling/docs — no Rust touched, so every binding/bench/docs section carried forward
    verbatim). Still partially met: Rust-core (boundary vectors + sweep + semver/v1.0.0 HELD) and
    CI/CD (ruff slices C/D). **Recommended next = ruff slice C** (unblocked); step (b) is contested
    — see below.
- **CI GREEN on origin/develop tip `cc838fc`**; HEAD `4cd963b` = +1 UNPUSHED log commit,
...
- **Open tension (135)**: iter-133 state.md said "start step (b) with a Rust-only fixture"; iter-134
    review handoff says "do NOT start (a2) or (b)" until the human ruling lands. Both defensible
    (the 3 boundary cases are single code points); 135 recommends slice C now, fixture only if still
    open.
- **Dep-refresh: ALL 7 slices DONE** (124-130) → `dep-refresh-survey.md`. **ruff 0.16**: A done 131
    (104→26), **B done 134** (`extend-select` S+C901 → all 15 RUF100 cleared; 26→**12** live: I001
    8, RUF022/RUF007/PLW1510/EXE001 1 each). **C next** = isort cluster (needs a
    `[tool.ruff.lint.isort]` `known-first-party`/`src` decision — `iscc_lib` lives under
    `crates/iscc-py/python/`) + the 3 one-liners; bundle `RUF100` into `extend-select`. D = drop the
    `ruff<0.16` pin once `uvx ruff@0.16.0 check .` exits 0. Then magnus 0.8 / jni 0.22 (source
    rewrites). **Never `ruff@0.16 check --fix .`** — it deletes load-bearing `# noqa`.
- **7 issues: 0 critical, 5 normal, 2 low — ONE `[review]` with HUMAN REVIEW REQUESTED** (new 133:
    ...
    ruling, not a redesign → `unicode-contract.md`; still open at 135). CID-doable: ruff C/D
    (unblocked), Unicode step (b) (contested). Human-gated: npm OIDC, single-registry re-trigger;
    low (CID skips) = v1.0.0 (HELD), docs logos.
- **Don't re-flag as new work** (all DONE): ruff slice B / project-wide S+C901 (134), Unicode freeze
    filter + generator (133), Kotlin 2.3+ floor docs in all 3 consumer docs (132), dep slices 1-7
    (124-130), aarch64 wheels #49 (123), CRAP baseline (122), trailing-byte fixes (120-121), Go IDv1
    #43 (119), WASM SIMD #42 (118), GIL #39+#41, cargo-deny (113), iai perf gate (107-111), semver
    gate (93). CID infra (audit role, metrics.jsonl, decisions.md) = meta, NOT target — ignore.
