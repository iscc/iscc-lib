...
## Current State (assessed-at: bd4eed5)

- **IN_PROGRESS**: all 13 CI jobs green (run 22714072781); **csbindgen P/Invoke layer complete**
- **v0.2.0 released** — all 8 registries including RubyGems (OIDC trusted publishing)
- **C#/.NET**: `NativeMethods.g.cs` (929 lines, 47 P/Invoke externs) auto-generated by csbindgen in
    `crates/iscc-ffi/build.rs`; `IsccLib.cs` still has only 1 wrapper (ConformanceSelftest)
- **CI (run 22714072781)**: ALL SUCCESS — 13 jobs ✅
...
- **packages/**: `go/` + `dotnet/` (partial); `cpp/`, `swift/`, `kotlin/` do NOT exist
- **Next action**: idiomatic C# wrappers in IsccLib.cs (all 32 symbols) + conformance xUnit tests
