<!-- assessed-at: 3a44652f1caa32e659c2c562f88cca2a90641f78 -->

# Project State

## Status: IN_PROGRESS

## Phase: C# idiomatic wrappers — 22 of 32 symbols complete

v0.2.0 released across all 8 registries. The C# binding expanded from 14 to 22 Tier 1 symbols: all
10 `gen_*_v0` functions, 2 encoding utilities, and the `SumCodeResult` record are now present. 10
symbols remain (algorithm primitives, codec ops, streaming classes), plus structured return types
for gen functions, conformance tests, docs, and NuGet publishing.
...
- WASM CI job = SUCCESS in run 22719988100
...
- `go.mod` updated to Go 1.26.1 (devcontainer dependency update, no functional change)
...
**Status**: partially met (22 of 32 symbols wrapped; structured return types, streaming classes,
algorithm primitives, codec ops, conformance tests, docs, NuGet publish all missing)
...
- `packages/dotnet/Iscc.Lib/IsccLib.cs` — 22 public symbols wrapped:
    ...
    - 1 result record: `SumCodeResult` (in IsccLib.cs top section)
- `packages/dotnet/Iscc.Lib.Tests/SmokeTests.cs` — **25 xUnit tests** (all pass in CI)
- `packages/dotnet/.gitignore` — excludes `bin/` and `obj/` artifacts
- `.devcontainer/Dockerfile` — .NET SDK 8 installed
- CI job `C# / .NET (dotnet build, test)` — SUCCESS in run 22719988100
...
- **Missing — spec requires structured record return types for gen functions**: 9 of 10 gen
    ...
    `DataCodeResult`, `InstanceCodeResult`, etc. (all defined in target spec's `Results.cs`). Only
    `SumCodeResult` is structured.
- **Missing — 10 of 32 Tier 1 symbols not yet wrapped**:
    - Algorithm primitives: `AlgMinhash256`, `AlgCdcChunks`, `AlgSimhash`, `SlidingWindow`
    - Soft hash: `SoftHashVideoV0`
    - Codec operations: `IsccDecode`, `IsccDecompose`, `EncodeComponent`
...
- **ALL PASSING** — latest CI run 22719988100: all **13 jobs** SUCCESS
- URL: https://github.com/iscc/iscc-lib/actions/runs/22719988100
...
**C#/.NET remaining work** — 22 of 32 symbols wrapped, CI green. Priority order:

1. **Structured result records** (`Results.cs`): `MetaCodeResult` (Iscc, Name, MetaHash,
    Description?, Meta?), `TextCodeResult` (Iscc, Characters), `DataCodeResult` (Iscc),
    `InstanceCodeResult` (Iscc, DataHash, FileSize) — refactor existing gen functions to return
    these instead of `string`
2. **Algorithm primitives**: `AlgMinhash256`, `AlgCdcChunks`, `AlgSimhash`, `SlidingWindow`
3. **Codec operations**: `IsccDecode`, `IsccDecompose`, `EncodeComponent`
4. **Soft hash**: `SoftHashVideoV0`
5. **Streaming classes** (`IsccDataHasher.cs`, `IsccInstanceHasher.cs`): implementing `IDisposable`,
    with `Update(ReadOnlySpan<byte>)` and `Finalize(uint bits = 64)`
6. **Conformance tests** (`ConformanceTests.cs` + vendored `testdata/data.json`)
