<!-- assessed-at: b92af0235fbb7e051e147df46ff90ae7833b32f7 -->

# Project State

## Status: IN_PROGRESS

## Phase: C# bindings — version sync complete; NuGet publish pipeline pending

v0.2.0 released across all 8 registries. C# / .NET documentation and version sync are now both
complete: `<Version>0.2.0</Version>` added to `Iscc.Lib.csproj` and `scripts/version_sync.py`
extended with `_get_csproj_version` / `_sync_csproj` targets (CI run 22804181997, all 13 jobs
green). Remaining C# gap: NuGet publish pipeline (`release.yml`). Open `normal`-priority issues
remain (C++ wrapper, NuGet pipeline).
...
- WASM CI job = SUCCESS in run 22804181997
...
types typed including streaming hashers; documentation complete; version sync complete; NuGet
publish pipeline still missing)
...
- CI job `C# / .NET (dotnet build, test)` — SUCCESS in run 22804181997
...
- Root `README.md` — NuGet badge, C# install tab, C# quickstart example ✅
- `zensical.toml` — "C# / .NET" nav entry in How-To Guides section ✅
- `scripts/version_sync.py` — `.csproj` version in TARGETS list with `_get_csproj_version` /
    `_sync_csproj` functions ✅
...
- `Bench (compile check)` CI job SUCCESS in run 22804181997

## CI/CD and Publishing

**Status**: met (for existing bindings; NuGet + C++/Swift/Kotlin publish not yet added)

- **ALL PASSING** — latest CI run 22804181997: all **13 jobs** SUCCESS
- URL: https://github.com/iscc/iscc-lib/actions/runs/22804181997
...
**NuGet publish pipeline** — version sync is now complete. The last major `normal`-priority item for
the C# bindings issue:
...
2. **SafeHandles.cs**: extract `SafeHandle` subclasses from `IsccDataHasher.cs` /
    `IsccInstanceHasher.cs` into `Native/SafeHandles.cs` per spec file layout (cosmetic refactor).
