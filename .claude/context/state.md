<!-- assessed-at: 20a4624518fc1c8ea20b5b3340739ab5c88c0b79 -->

# Project State

## Status: IN_PROGRESS

## Phase: C# idiomatic wrappers — 30 of 32 symbols complete

v0.2.0 released across all 8 registries. The C# binding advanced from 26 to 30 Tier 1 symbols this
iteration: `AlgSimhash`, `AlgMinhash256`, `AlgCdcChunks`, and `SoftHashVideoV0` added with proper
`IsccByteBuffer`/`IsccByteBufferArray` marshaling helpers. 2 streaming classes remain
(`IsccDataHasher`, `IsccInstanceHasher`), plus structured return types for gen functions,
conformance tests, docs, and NuGet publishing.
...
- WASM CI job = SUCCESS in run 22798685708
...
**Status**: partially met (30 of 32 symbols wrapped; streaming classes, structured return types,
conformance tests, docs, NuGet publish all missing)
...
- `packages/dotnet/Iscc.Lib/IsccLib.cs` — **30 public symbols wrapped**:
    ...
    - 4 algorithm primitives: `AlgSimhash`, `AlgMinhash256`, `AlgCdcChunks`, `SoftHashVideoV0` (**new
        this iteration**)
    - 1 diagnostic: `ConformanceSelftest`
    - 2 result records: `SumCodeResult`, `DecodeResult`
- Private helpers added: `ConsumeByteBuffer`, `ConsumeByteBufferArray` (follow `ConsumeNativeString`
    pattern — null check + data copy + free in finally)
- `packages/dotnet/Iscc.Lib.Tests/SmokeTests.cs` — **35 xUnit tests** (29 previous + 6 new)
- `packages/dotnet/.gitignore` — excludes `bin/` and `obj/` artifacts
- `.devcontainer/Dockerfile` — .NET SDK 8 installed
- CI job `C# / .NET (dotnet build, test)` — SUCCESS in run 22798685708
...
- **Missing — 2 of 32 Tier 1 symbols not yet wrapped**:
    - Streaming classes: `IsccDataHasher`, `IsccInstanceHasher` (with `IDisposable` + SafeHandle +
        Update/Finalize)
...
- `Bench (compile check)` CI job SUCCESS in run 22798685708

## CI/CD and Publishing

**Status**: met (for existing bindings; NuGet + C++/Swift/Kotlin publish not yet added)

- **ALL PASSING** — latest CI run 22798685708: all **13 jobs** SUCCESS
- URL: https://github.com/iscc/iscc-lib/actions/runs/22798685708
...
**C#/.NET streaming types** — 30 of 32 symbols wrapped, CI green. Priority order:

1. **Streaming classes** (`IsccDataHasher.cs`, `IsccInstanceHasher.cs`): `IDisposable` +
    `SafeHandle` pattern, `Update(ReadOnlySpan<byte>)` and `Finalize(uint bits = 64)`. Completes
    32/32 Tier 1 symbols.
2. **Structured result records** (`Results.cs`): `MetaCodeResult`, `TextCodeResult`,
    `DataCodeResult`, `InstanceCodeResult` — refactor 9 gen functions to return records instead of
    `string`.
3. **Conformance tests** (`ConformanceTests.cs` + vendored `testdata/data.json`)
4. **Documentation**: `docs/howto/dotnet.md`, `packages/dotnet/README.md`, README C# section
5. **NuGet publish job** in `release.yml`
