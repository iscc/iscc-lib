...
## Phase: Rust core Tier 1 complete — Python bindings 20/23, other bindings gen-only
...
expose 20 of 23 Tier 1 symbols (added `alg_simhash`, `alg_minhash_256`, `alg_cdc_chunks`). 250 Rust
tests and 116 Python tests pass. CI green.
...
- **Python bindings (`crates/iscc-py/`)**: 20 functions (9 gen with `IsccResult(dict)` returns,
    `conformance_selftest`, 4 text utils, 3 algo primitives, `encode_base64`, `iscc_decompose`,
    `sliding_window`), BinaryIO streaming, 116 tests, `ty` clean
...
- **Python bindings (3 remaining)**: `soft_hash_video_v0`, `DataHasher`, `InstanceHasher`
...
Complete Python bindings to 23/23 Tier 1 symbols. Remaining 3: `soft_hash_video_v0` (pure function,
straightforward), `DataHasher` and `InstanceHasher` (PyO3 `#[pyclass]` wrappers with
`update()`/`finalize()` methods — more complex than the `#[pyfunction]` wrappers done so far).
