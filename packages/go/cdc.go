// Pure Go implementation of Content-Defined Chunking (CDC) for ISCC.
// Uses a FastCDC-inspired gear rolling hash algorithm to split byte data into
...
// algCdcParams calculates CDC parameters from a target average chunk size.
// Returns (minSize, maxSize, centerSize, maskS, maskL).
func algCdcParams(avgSize uint32) (mi, ma, cs int, maskS, maskL uint32) {
	minSize := avgSize / 4
	maxSize := avgSize * 8
	// Integer ceiling division: (minSize + 1) / 2
	offset := int(minSize) + int((minSize+1)/2)
	centerSize := int(avgSize) - offset
	bits := uint32(math.Round(math.Log2(float64(avgSize))))
	maskS = (1 << (bits + 1)) - 1
	maskL = (1 << (bits - 1)) - 1
	return int(minSize), int(maxSize), centerSize, maskS, maskL
}

// algCdcOffset finds the CDC cut point offset within a buffer.
// Uses a gear rolling hash in two phases: strict mask then relaxed mask.
// Returns the byte offset of the cut point.
func algCdcOffset(buffer []byte, mi, ma, cs int, maskS, maskL uint32) int {
	var pattern uint32
	size := len(buffer)
	i := mi
	if i > size {
	...
	for i < barrier {
		pattern = (pattern >> 1) + cdcGear[buffer[i]]
		if pattern&maskS == 0 {
			return i + 1
		}
		i++
...
func AlgCdcChunks(data []byte, utf32 bool, avgChunkSize uint32) [][]byte {
	...
	mi, ma, cs, maskS, maskL := algCdcParams(avgChunkSize)
	var chunks [][]byte
	pos := 0

	for pos < len(data) {
		remaining := data[pos:]
		cutPoint := algCdcOffset(remaining, mi, ma, cs, maskS, maskL)
