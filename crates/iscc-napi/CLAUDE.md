...
- Wraps Tier 1 API as `#[napi]` exports: 5 constants, 10 code generators, 3 codec functions, 4 text
    utilities, 4 algorithm primitives, 2 streaming hasher classes, and 4 helper functions
...
crates/iscc-napi/
  ...
  __tests__/
    ...
    functions.test.mjs     # non-gen functions, streaming hashers, constants, gen_sum_code_v0
...
| `u8`                           | `number`                                      | Header fields in encode/decode codec functions        |
| `i64`                          | `number`                                      | Filesize in `NapiSumCodeResult`                       |
| `#[napi(object)] struct`       | plain JS object                               | `IsccDecodeResult`, `NapiSumCodeResult`               |
...
- **Conformance tests** (`conformance.test.mjs`): parametrized over vendored `data.json` vectors at
    `crates/iscc-lib/tests/data.json` -- each of the 9 `gen_*_v0` functions must match the `iscc`
    output field for every test case (gen_sum_code_v0 is tested in functions.test.mjs)
- **Function tests** (`functions.test.mjs`): unit tests for text utils, codec functions
    (`encode_component`, `iscc_decode`, `json_to_data_url`), `encode_base64`, `iscc_decompose`,
    `conformance_selftest`, `sliding_window`, algorithm primitives, `soft_hash_video_v0`, streaming
    hashers (`DataHasher`, `InstanceHasher`), `gen_sum_code_v0`, and all 5 exported constants
...
- Build matrix targets (CI builds all 5; `package.json` napi config declares defaults +
    `aarch64-unknown-linux-gnu`, CI adds `aarch64-apple-darwin`):
...
- **Do not expose Rust enums** directly via napi -- they break when variants are added; use strings
    or integers instead
- **Prefer returning strings for gen functions** -- most `gen_*_v0` bindings return only the `iscc`
    string field from result structs (e.g., `r.iscc`); use `#[napi(object)]` structs only when
    multiple fields are needed (e.g., `IsccDecodeResult`, `NapiSumCodeResult`)
