...
fn is_unassigned_in_unicode16(c: char) -> bool {
    ...
    in_ranges(cp, &unicode16::UNASSIGNED_RANGES)
}

/// Binary-search a sorted, non-overlapping table of inclusive `(lo, hi)` ranges.
#[cfg(feature = "text-processing")]
...
/// Check whether a character is `Cased` (and not `Case_Ignorable`) in Unicode 16.0.0.
///
/// Backed by the vendored range table in [`unicode16_case`], generated by
/// `scripts/gen_unicode16_case.py` from CPython 3.14's Unicode 16.0.0 data. The
/// exclusion of `Case_Ignorable` code points is unobservable: the `Final_Sigma`
/// scan skips case-ignorables before ever testing casedness.
#[cfg(feature = "text-processing")]
fn is_cased_in_unicode16(c: char) -> bool {
    in_ranges(c as u32, &unicode16_case::CASED_RANGES)
}

/// Check whether a character is `Case_Ignorable` in Unicode 16.0.0.
///
/// Backed by the vendored range table in [`unicode16_case`], generated by
/// `scripts/gen_unicode16_case.py` from CPython 3.14's Unicode 16.0.0 data.
#[cfg(feature = "text-processing")]
fn is_case_ignorable_in_unicode16(c: char) -> bool {
    in_ranges(c as u32, &unicode16_case::CASE_IGNORABLE_RANGES)
}

/// For each position, whether the first non-`Case_Ignorable` character at or after
/// it is `Cased`, per the vendored Unicode 16.0.0 tables.
///
/// Returns `chars.len() + 1` entries; the final entry is `false` (end of string).
#[cfg(feature = "text-processing")]
fn cased_lookahead(chars: &[char]) -> Vec<bool> {
    let mut followed_by_cased = vec![false; chars.len() + 1];
    for (i, &c) in chars.iter().enumerate().rev() {
        followed_by_cased[i] = if is_case_ignorable_in_unicode16(c) {
            followed_by_cased[i + 1]
        } else {
            is_cased_in_unicode16(c)
        };
    }
    followed_by_cased
}

/// Lowercase text with the `Final_Sigma` condition frozen at Unicode 16.0.0.
///
/// `str::to_lowercase()` decides `Final_Sigma` from the Unicode tables the Rust
/// compiler ships (rustc 1.97: 17.0, which reclassified U+0295 from `Ll` to `Lo`),
/// making output a function of the rustc version. This wrapper pre-substitutes
/// every U+03A3 with its context-decided lowercase form — U+03C2 iff the nearest
/// preceding non-`Case_Ignorable` character is `Cased` and the nearest following
/// non-`Case_Ignorable` character is not `Cased` (or absent) — using the vendored
/// 16.0.0 tables, then delegates to `to_lowercase()`. Std never sees a U+03A3, so
/// its own `Final_Sigma` branch can never fire, while every other full-case
/// mapping (e.g. U+0130) is still handled by std.
#[cfg(feature = "text-processing")]
fn to_lowercase_unicode16(text: &str) -> String {
    const CAPITAL_SIGMA: char = '\u{03A3}';
    if !text.contains(CAPITAL_SIGMA) {
        return text.to_lowercase(); // hot path unchanged
    }
    let chars: Vec<char> = text.chars().collect();
    let followed_by_cased = cased_lookahead(&chars);
    let mut out = String::with_capacity(text.len());
    let mut last_non_ignorable: Option<char> = None;
    for (i, &c) in chars.iter().enumerate() {
        if c == CAPITAL_SIGMA {
            let final_sigma =
                last_non_ignorable.is_some_and(is_cased_in_unicode16) && !followed_by_cased[i + 1];
            out.push(if final_sigma { '\u{03C2}' } else { '\u{03C3}' });
        } else {
            out.push(c);
        }
        if !is_case_ignorable_in_unicode16(c) {
            last_non_ignorable = Some(c);
        }
    }
    out.to_lowercase()
}
...
/// Replaces code points unassigned in Unicode 16.0.0 (the declared Unicode data
/// version) with the noncharacter sentinel `U+FFFF` before normalization, applies
/// NFD normalization, lowercasing (with the `Final_Sigma` condition frozen at
/// Unicode 16.0.0 via [`to_lowercase_unicode16`]), removes whitespace and
/// characters in Unicode categories C (control), M (mark), and P (punctuation),
/// then recombines with NFKC normalization.
///
...
pub fn text_collapse(text: &str) -> String {
    ...
    //    and lowercase with the Final_Sigma condition frozen at Unicode 16.0.0
    let nfd: String = text
        .chars()
        .map(|c| {
            if is_unassigned_in_unicode16(c) {
        ...
        .nfd()
        .collect();
    let nfd_lower = to_lowercase_unicode16(&nfd);
...
mod tests {
    ...
    // Expected values in the Final_Sigma case-freeze tests below are the oracle:
    // iscc-core 1.3.0 on CPython 3.14 (unicodedata 16.0.0). U+0295 is `Ll`
    // (Cased) in Unicode 16.0.0 but `Lo` (not Cased) in 17.0 — the vendored
    // tables, not the compiler's, must decide Final_Sigma.

    #[cfg(feature = "text-processing")]
    #[test]
    fn test_final_sigma_following_context_cased_in_16() {
        // Σ before U+0295 (cased in 16.0, not in 17.0) stays medial.
        assert_eq!(
            text_collapse("\u{0391}\u{03A3}\u{0295}\u{0392}"),
            "\u{03B1}\u{03C3}\u{0295}\u{03B2}"
        );
    }

    #[cfg(feature = "text-processing")]
    #[test]
    fn test_final_sigma_preceding_context_cased_in_16() {
        // Σ after U+0295 (cased in 16.0, not in 17.0) goes final.
        assert_eq!(text_collapse("\u{0295}\u{03A3}"), "\u{0295}\u{03C2}");
    }

    #[cfg(feature = "text-processing")]
    #[test]
    fn test_final_sigma_other_lowercase_is_cased() {
        // U+2170 (SMALL ROMAN NUMERAL ONE) is cased via Other_Lowercase, so the
        // sigma stays medial; NFKC then maps U+2170 to `i`.
        assert_eq!(
            text_collapse("\u{0391}\u{03A3}\u{2170}"),
            "\u{03B1}\u{03C3}\u{0069}"
        );
    }

    #[cfg(feature = "text-processing")]
    #[test]
    fn test_final_sigma_modifier_letter_is_case_ignorable() {
        // U+02B0 (MODIFIER LETTER SMALL H, Lm) is case-ignorable: the scan
        // skips it and finds the cased U+0392 after the sigma.
        assert_eq!(
            text_collapse("\u{0391}\u{03A3}\u{02B0}\u{0392}"),
            "\u{03B1}\u{03C3}\u{0068}\u{03B2}"
        );
    }

    #[cfg(feature = "text-processing")]
    #[test]
    fn test_final_sigma_apostrophe_is_case_ignorable() {
        // U+0027 (APOSTROPHE) is case-ignorable via Word_Break=MidLetter.
        assert_eq!(
            text_collapse("\u{0391}\u{03A3}\u{0027}\u{03B2}"),
    ...
    #[cfg(feature = "text-processing")]
    #[test]
    fn test_final_sigma_plain_word_end() {
        // Plain final sigma at the end of a cased word (passes pre-freeze too).
        assert_eq!(
            text_collapse("\u{039B}\u{039F}\u{0393}\u{039F}\u{03A3}"),
            "\u{03BB}\u{03BF}\u{03B3}\u{03BF}\u{03C2}"
        );
    }

    #[cfg(feature = "text-processing")]
    #[test]
    fn test_unicode16_case_table_invariants() {
        use super::unicode16_case::{CASE_IGNORABLE_RANGES, CASED_RANGES};
        assert_eq!(CASED_RANGES.len(), 152);
        assert_eq!(CASE_IGNORABLE_RANGES.len(), 452);
        for (ranges, expected_total) in [
            (&CASED_RANGES[..], 4_311),
            (&CASE_IGNORABLE_RANGES[..], 2_749),
        ] {
            let mut covered: u32 = 0;
        ...
        // U+0295 must be Cased (the 16.0 vs 17.0 divergence this table fixes);
        // the sentinel U+FFFF must be in neither table.
        assert!(is_cased_in_unicode16('\u{0295}'));
        assert!(!is_cased_in_unicode16('\u{FFFF}'));
        assert!(!is_case_ignorable_in_unicode16('\u{FFFF}'));
    }
